...
function showGraph() {
  ...
  for (let x = -width / 2; x <= width /2; x += yStep) {
    //let y = amplitudeModulation( triangularFunc, squareFunc, x);
    let y = frequencyModulation(sinFunc2, sinFunc, x, 180);
...
function sinFunc2(x, amp = 180, freq = 10, ph = 0) {
  return amp * sin((5 * freq * x / N) + ph);
}
